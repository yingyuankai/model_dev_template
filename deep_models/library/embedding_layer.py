...
class EmbeddingLayer(Layer):
    ...
    def __init__(self, vocab_size, emb_size, name="embedding", 
        ...
        with tf.variable_scope('embedding'), tf.device('/cpu:0'):
            ...
            self._W = tf.Variable(
                tf.constant(0., shape=[vocab_size, emb_size]),
                trainable=False, name=name + '_W')
    ...
    def assign_pretrain_embedding(self, pretrain_embedding):
        with tf.variable_scope("assign_pretrain_embedding"):
            embedding_init = tf.assign(self._W, pretrain_embedding)
...
class MultiRegionEmbeddingLayer(EmbeddingLayer):
    ...
    def __init__(self, vocab_size, emb_size, region_sizes, \
        ...
        for i in range(region_num - 1):
            st = int(self._region_sizes[-1]/2) - int(self._region_sizes[i]/2)
