...
def init():
    f = open("./data/raw/cail_0518/law.txt", 'r', encoding='utf8')
    law = {}
    lawname = {}
    line = f.readline()
    while line:
    ...
    f = open("./data/raw/cail_0518/accu.txt", 'r', encoding='utf8')
    accu = {}
    accuname = {}
    line = f.readline()
    while line:
...
def getlabel(d, kind):
    ...
    if kind == 'relevant_articles':
        return [law[str(law_itm)] for law_itm in d['relevant_articles']]
    if kind == 'accusation':
        return [accu[str(acc_itm)] for acc_itm in d['accusation']]

    if kind == 'term_of_imprisonment':
        return gettime(d['term_of_imprisonment'])
