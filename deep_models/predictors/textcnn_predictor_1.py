...
from utils.math_utils import softmax, sigmoid, softmax_new
...
class Predictor(BasePredict):
    ...
    def _load_models(self):
        try:
            model_path = os.path.join(self._config.model_output_path, "model_{}{}{}_{}_{}_{}_{}_{}{}{}.pb".format(int(self._config.rebalance),
                                                                      int(self._config.use_label_loss_weight),
                                                                      int(self._config.use_prior),
                                                                      int(self._config.max_seq_len),
                                                                      str(self._config.learning_rate),
                                                                      str(self._config.decay_steps),
                                                                      "".join([str(tmp) for tmp in self._config.task_loss_weights]),
                                                                      str(self._config.embed_size),
                                                                      "_avg" if self._config.avgeraged else "",
                                                                      "_big" if self._config.use_wenshu_emb else ""))
            # model_path = "model_111_2018070103.pb"
    ...
    def predict(self, content):
        ...
        # impris_time = list(range(0, 301))
        # impris_time.extend([-1, -2])
        # impris_time = np.array(impris_time)
        # impris = softmax_new(impris_logits)
        # impris = np.sum(np.multiply(impris, impris_time), axis=1)
        # impris = np.round(impris)
        death = np.argmax(death_logits, axis=1)

        for i in range(len(law_logits)):
            result_tmp = {
                "imprisonment": int(impris[i]) if int(death[i]) == 0 else int(death[i]) * -1,
