...
from utils.nn_utils import list_to_multi_hot, list_to_double_multi_hot, id_to_value
...
class Trainer(BaseTrain):
    ...
    def train_step(self):
        ...
        # laws = list_to_double_multi_hot(laws_batch, len(self.config.data_obj.law2id))
        accus = list_to_multi_hot(accus_batch, len(self.config.data_obj.accu2id))
        # accus = list_to_double_multi_hot(accus_batch, len(self.config.data_obj.accu2id))
        impris = id_to_value(impris_batch, self.config.data_obj.id2imprisonment)
        death = death_batch
