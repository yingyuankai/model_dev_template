...
class Model(BaseModel):
    ...
    def inference(self):
        ...
        [batch_size, max_seq_len, embed_size]     embedding projecting
        [batch_size, max_seq_len, 1, channel_size]   region_embedding
        [batch_size, max_seq_len, 1, channel_size]   conv block
        [batch_size, max_seq_len, 1, channel_size]   shortcut

        for i in range(num_repeat):
            [batch_size, max_seq_len / 2 ** (i + 1), 1, channel_size]   downsampling
            [batch_size, max_seq_len / 2 ** (i + 1), 1, channel_size]   conv block
            [batch_size, max_seq_len / 2 ** (i + 1), 1, channel_size]   shortcut

        [batch_size, 1, 1, channel_size]   max pooling
        [batch_size, channel_size]   flatten

        :return:
        """
        inputs = tf.nn.embedding_lookup(self.embedding, self.facts)
        feature_obj = self.config.Feature(self.config)
        net_flatted = feature_obj.feature(inputs, self.is_training)
        ...
        with tf.name_scope("region_embedding"):
            with slim.arg_scope([slim.conv2d], padding='VALID', normalizer_fn=slim.batch_norm,
                ...
                net = slim.conv2d(inputs,
                                  ...
                                  [self.config.filter_size, self.config.embed_size],
                                  scope='conv_{}'.format(self.config.filter_size))
                return net

    def conv_block(self, inputs, name, max_pooling=True):
        if max_pooling:
            ...
            with tf.variable_scope("{}_conv_{}".format(name, i)):
                with slim.arg_scope([slim.conv2d],
                                    ...
                                    normalizer_params={'is_training': self.is_training,
                                                       "scale": True,
                                                       "epsilon": 1e-5}):
                    inputs = slim.conv2d(inputs, self.config.channel_size, [3, inputs.get_shape()[2]])
